            ...
            case LLM_ARCH_CHATGLM:
                {
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        ...
                        layer.w2 = ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_DOWN, "weight", i), {n_ff, n_embd}, backend_split);
                        layer.w3 = ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_UP,   "weight", i), {n_embd, 2 * n_ff}, backend_split);

                        if (backend == GGML_BACKEND_GPU) {
                            vram_weights +=
                                ggml_nbytes(layer.attn_norm) + ggml_nbytes(layer.attn_norm_2) + ggml_nbytes(layer.wqkv) + ggml_nbytes(layer.wo) + ggml_nbytes(layer.w2) + ggml_nbytes(layer.w3);
                        }
                    }
                } break;
            case LLM_ARCH_FALCON:
...
static struct ggml_cgraph * llm_build_chatglm(
         llama_context & lctx,
     const llama_batch & batch) {
    const auto & model   = lctx.model;
    const auto & hparams = model.hparams;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // feed-forward network
        {
            ...
            const size_t wsize = ggml_type_size(cur->type);
            struct ggml_tensor * tmp_1 = ggml_view_2d(ctx0, tmp, n_ff, n_tokens, wsize*2*n_ff, 0);
            struct ggml_tensor * tmp_2 = ggml_view_2d(ctx0, tmp, n_ff, n_tokens, wsize*2*n_ff, wsize * n_ff);

            // SILU activation
            cur = ggml_silu(ctx0, tmp_1);
            offload_func(cur);
            ggml_set_name(cur, "silu");

            cur = ggml_mul(ctx0, cur, tmp_2);
            offload_func(cur);
            ggml_set_name(cur, "silu_x_result_w3");
...
static struct ggml_cgraph * llama_build_graph(
    ...
    switch (model.arch) {
        ...
        case LLM_ARCH_CHATGLM:
            {
                result = llm_build_chatglm(lctx, batch);
            } break;
        case LLM_ARCH_FALCON:
            {
